...
export default function Test8() {
    ...
    const handleMode = useCallback(() => {
        ...
        const tr = view.state.tr.setDocAttribute('theme', newTheme);
        // 트랜잭션의 meta는 필요에 따라 설정 가능
        tr.setMeta('addToHistory', false); // history에 추가하지 않음
    ...
    useEffect(() => {
        ...
        const basicSchema = new Schema({
            ...
            nodes: {
                doc: {
                    content: 'block+',
                    attrs: { theme: { default: 'light', validate: 'string' } },
                    toDOM(node) {
                        console.log('toDom', node);
                        // doc에서는 해당 함수 실행하지 않음
                        throw new Error('doc 안에서 toDOM 함수 실행');
                    },
                },
                paragraph: {
                    group: 'block',
            ...
            return new Plugin({
                ...
                view(editorView) {
                    ...
                    const updateTheme = (theme?: string) => {
                        if (theme === 'dark') {
                            editorView.dom.parentElement?.parentElement?.classList.add('dark');
                        } else {
                            editorView.dom.parentElement?.parentElement?.classList.remove('dark');
                        }
                    };
                    updateTheme(editorView.state.doc.attrs.theme);
    ...
    return (
        <div className="p-4 h-full w-full">
            <div className="flex w-full items-center">
                <div>test-prosemirror(dark-mode, doc의 attrs에 따른 class 주입삭제)</div>
